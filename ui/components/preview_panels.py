...
def render_preview_panels(render_projection: RenderProjection) -> None:
    ...
    with mask_tab:
        _render_view_placeholder(
            ...
            summary_lines=[
                ...
                f"Threshold mirror: {render_projection.mask_threshold:.2f}",
                f"Feather mirror: {render_projection.mask_feather}",
                f"Invert mask: {'On' if render_projection.mask_invert else 'Off'}",
            ],
            inspection_lines=[
    ...
    with preview_tab:
        debug_label = "enabled" if render_projection.show_debug_overlay else "disabled"
        _render_view_placeholder(
            title="Masked Preview",
            summary_lines=[
                "Display mode: composited preview placeholder.",
                f"Preview generation counter: {render_projection.preview_generation}",
...
def render_workbench_frame_panel(render_projection: RenderProjection) -> None:
    """Render the dedicated workbench frame panel inside the stable workbench host."""
    _render_mask_workbench_panel(render_projection)
...
def _render_mask_workbench_panel(render_projection: RenderProjection) -> None:
    workbench_state = "backend frame bound" if render_projection.video_loaded else "waiting for source asset"
    overlay_state = "debug overlay on" if render_projection.show_debug_overlay else "debug overlay off"
    empty_lines = [
    ...
    if render_projection.frame_error_message:
        empty_lines.insert(0, f"Frame loading error: {render_projection.frame_error_message}")

    render_workspace_image_panel(
        title="Mask Workbench",
        metadata_items=[
            ("Frame", f"{render_projection.frame_index:04d}"),
            ("Timecode", format_timecode(render_projection.timestamp_seconds)),
            ("Overlay", overlay_state),
            ("State", workbench_state),
        ],
        image_bytes=render_projection.frame_image_bytes,
        mime_type=render_projection.frame_image_mime_type,
        empty_title="Mask Authoring Workspace",
        empty_lines=empty_lines,
        panel_note=(
            f"Workbench frame source: {render_projection.video_name} | "
            "Future use: prompt interaction, overlay editing, mask layer review."
            if render_projection.video_loaded
